...
void rectPressed() {
    if (rectButton_font.press()) {
        ...
        println("Access this view directly using the following link:\n\n//bar.utoronto.ca/geneslider/?datasource=" + source + "&cistome=" + cistome + "&agi=" + agi + "&before=" + before + "&after=" + after + "&zoom_from=" + startDigit + "&zoom_to=" + endDigit
...
void saveScreenGrab() {
    ...
    textAlign(LEFT);
    fill(#888888);
    text("Please cite Waese et al., (manuscript in preparation). Gene Slider - //bar.utoronto.ca/geneslider", 20, 575); 
...
String goUpstream() {
    if (JS) {
        String query = getQuery();
        before = before + 1000;
        String link = "//bar.utoronto.ca/geneslider/?datasource=" + source + "&agi=" + agi + "&before=" + before + "&after=" + after + "&zoom_from=" + startDigit + "&zoom_to=" + endDigit
            + "&weightedBitscore=" + showWeightedBitScore + "&alnIndicator=" + alignmentCountIndicator + query;
        return link;
    } else {
        return "";
