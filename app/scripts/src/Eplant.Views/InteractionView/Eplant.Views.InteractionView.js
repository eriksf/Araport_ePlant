(function() {
	...
	*/
	...
	Eplant.Views.InteractionView.interactionsThreshold = 300;	// If the number of interactions exceeds this number, a filter will be applied automatically
	...
	*/
	Eplant.Views.InteractionView.initialize = function() {
		...
		Eplant.Views.InteractionView.domCacheContainer = document.getElementById("Cytoscape_cache");	// DOM container element for Cytoscape
	};
	...
	*/
	Eplant.Views.InteractionView.prototype.createViewSpecificUIButtons = function() {
		...
		function(data) {		// click
			window.open("http://bar.utoronto.ca/interactions/cgi-bin/arabidopsis_interactions_viewer.cgi?interactions=yes&input=" + data.interactionView.geneticElement.identifier + "&qbar=yes");
		},
		{
			interactionView: this
		}
	...
	*/
	Eplant.Views.InteractionView.prototype.setCyConf = function() {
		...
		this.cyConf.ready  = $.proxy(function() {
			...
			this.queryNode = this.cy.nodes("#" + this.geneticElement.identifier.toUpperCase());
				...
				if (geneticElement) {		// GeneticElement exists
					...
					if (!geneticElement.geneticElementDialog) {
						...
						if (!geneticElement.geneticElementDialog) {		// No
							...
							var position2 = {
								x:position.x + 200,
								y: position.y -80-node.renderedHeight()
							};
