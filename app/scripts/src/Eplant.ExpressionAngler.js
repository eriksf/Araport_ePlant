(function() {
	...
	*/
	Eplant.ExpressionAngler = function(URL, mainIdentifier,resultCount) {
		...
		setTimeout( progress, 1000 );
		Eplant.expressionXhrService = $.get(URL, $.proxy(function( data ) {
			...
			if(tempLinkEle.length>0){
				...
				Eplant.expressionXhrLink = $.get( tempFileLink,  $.proxy(function( list ) {
					...
					this.showFoundGenes(list,mainGeneticElement,URL,resultCount);
	...
	Eplant.showFoundGenes = function(list,mainGeneticElement,URL,resultCount) {
		var dom = $('<div/>', {
		});
		dom.css({'text-align':'center'});
		var p = $('<div/>', {
			html: '<pre style="font-weight:bold">Results:</pre> <br> Expression Angler found '+list.length+' matches. <br>'
			}).css({
			'max-height': '400px',
			'overflow': 'auto'
		});
		...
		if(list.length>0){
			...
			continueButton.css({'margin-top':'10px',width:'170px'});
			continueButton.on('click',$.proxy(function(){
				...
				this.EA.loadExpressionAnglerGenes(selectedList,this.mainGeneticElement);
				if(Eplant.expressionAnglerResultDialog) Eplant.expressionAnglerResultDialog.close();
			},{list:list,mainGeneticElement:mainGeneticElement,EA:this}));
			$(dom).append(continueButton);
			var moreGeneButton = $('<input/>', {
				type: 'button',
				value: "Get more genes",
				'class':'button greenButton'
			});
			moreGeneButton.css({'margin-top':'10px',width:'170px'});
			moreGeneButton.on('click',$.proxy(function(){
				...
				Eplant.ExpressionAngler(URL, this.EA.mainIdentifier,resultCount);
			},{list:list,mainGeneticElement:mainGeneticElement,EA:this}));
